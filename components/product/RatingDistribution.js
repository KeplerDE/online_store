...
export default function RatingDistribution({ reviews }) {
  // Initialize rating distribution and total review count
  const distribution = { 5: 0, 4: 0, 3: 0, 2: 0, 1: 0 };
  let totalReviews = 0;

  // Accumulate rating distribution and total review count
  reviews.forEach(review => {
    distribution[review.rating]++;
    totalReviews++;
  });

  // Generate rating icons with percentage for each rating
  const ratingIcons = Object.keys(distribution).map(rating => {
    ...
    const starIcons = Array.from({ length: parseInt(rating) }, (_, index) => <FaStar key={index} className="text-danger" />);
    const emptyStarIcons = Array.from({ length: 5 - parseInt(rating) }, (_, index) => <FaRegStar key={index} />);
    ...
    return (
      <div key={rating} className="d-flex justify-content-between align-items-center">
        <div className="progress col-6 p-0 m-0 mt-1" style={{ height: "10px" }}>
          <div className="progress-bar bg-secondary" role="progressbar" style={{ width: `${percentage}%` }} aria-valuenow={percentage} aria-valuemin="0" aria-valuemax="100"></div>
        </div>
        <div className="col-6">
